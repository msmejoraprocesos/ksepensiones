...
const DS = {
  ...
  card: { background: 'white', borderRadius: '14px', border: '1px solid #E1E7F0', padding: 'clamp(14.0px, 1.5vw, 19.4px) clamp(14.0px, 1.67vw, 21.1px)', marginBottom: '16px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)' } as React.CSSProperties,
  cardHighlight: { background: 'white', borderRadius: '14px', padding: 'clamp(14.0px, 1.5vw, 19.4px) clamp(14.0px, 1.67vw, 21.1px)', marginBottom: '16px', boxShadow: '0 0 0 2px #14375F, 0 4px 14px rgba(20,55,95,0.12)' } as React.CSSProperties,
  ...
  tCell: { padding: '11px 14px', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#132135', borderBottom: '1px solid #E1E7F0' } as React.CSSProperties,
  tCellR: { padding: '11px 14px', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#132135', borderBottom: '1px solid #E1E7F0', textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums' as const } as React.CSSProperties,
  tCellBold: { padding: '11px 14px', fontSize: 'clamp(13px, 1.00vw, 16px)', color: '#245287', fontWeight: 700, borderBottom: '1px solid #E1E7F0', textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums' as const } as React.CSSProperties,
  tRowAlt: (i: number) => ({ background: i % 2 === 0 ? 'white' : '#F8FAFC' }) as React.CSSProperties,
  secTitle: { fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: 700, color: '#132135', margin: '0 0 16px', letterSpacing: '-0.015em' } as React.CSSProperties,
  kpiBlock: { background: '#0D2440', borderRadius: '12px', padding: 'clamp(11.4px, 1.23vw, 15.8px) clamp(14.0px, 1.5vw, 19.4px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' } as React.CSSProperties,
  kpiGreen: { background: '#12855C', borderRadius: '12px', padding: 'clamp(11.4px, 1.23vw, 15.8px) clamp(14.0px, 1.5vw, 19.4px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' } as React.CSSProperties,
  ...
  criticalNum: { fontSize: 'clamp(14.5px, 1.27vw, 20.2px)', fontWeight: '800' as const, letterSpacing: '-0.5px', lineHeight: 1 } as React.CSSProperties,
  bigNum: { fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: '700' as const, letterSpacing: '-0.5px' } as React.CSSProperties,
}
// ──────────────────────────────────────────────────────────────────
...
function SimuladorVidaReal({ pensionSin, pensionCon }: { pensionSin: number; pensionCon: number }) {
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #E5E7EB', marginTop: '4px' }}>
      <div style={{ background: AZUL, padding: 'clamp(10px, 1.2vw, 14px) clamp(11.4px, 1.23vw, 15.8px)' }}>
        <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#93C5FD', margin: '0 0 2px', fontWeight: '700' }}>
          Simulador de vida real
        </p>
        <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '800', color: 'white', margin: 0 }}>
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
              ...
              <div key={g.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                    <label style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#64748B' }}>{g.label.substring(3)}</label>
                    <span style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: '700', color: g.color }}>{fmtMXN(g.val)}</span>
                  </div>
        ...
        <div style={{ background: '#F8FAFC', borderRadius: '10px', padding: '14px' }}>
          <p style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: '700', color: '#374151', margin: '0 0 12px' }}>
            Comparativa
          </p>
          ...
          {[
            ...
            <div key={i} style={{ marginBottom: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                <span style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#374151' }}>{b.label}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '8px' }}>
          <div style={{ padding: '12px', background: excedenteSin >= 0 ? '#F0FDF4' : '#FEF2F2', borderRadius: '10px', borderLeft: `3px solid ${excedenteSin >= 0 ? VERDE : ROJO}` }}>
            <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#64748B', margin: '0 0 4px' }}>Sin Modalidad 40</p>
            <p style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: '800', color: excedenteSin >= 0 ? VERDE : ROJO, margin: '0 0 2px' }}>
              {excedenteSin >= 0 ? `+${fmtMXN(excedenteSin)}` : fmtMXN(excedenteSin)}
            </p>
            <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#64748B', margin: 0 }}>
          ...
          <div style={{ padding: '12px', background: excedenteCon >= 0 ? '#F0FDF4' : '#FEF2F2', borderRadius: '10px', borderLeft: `3px solid ${excedenteCon >= 0 ? VERDE : ROJO}` }}>
            <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#64748B', margin: '0 0 4px' }}>Con Modalidad 40</p>
            <p style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: '800', color: excedenteCon >= 0 ? VERDE : ROJO, margin: '0 0 2px' }}>
        ...
        <div style={{ padding: '12px 14px', background: '#EEF2F8', borderRadius: '10px', borderLeft: `3px solid ${AZUL}` }}>
          ...
          <p style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#64748B', margin: 0 }}>
        ...
        {evaluaciones.map((e: any) => (
          <div key={e.id} style={{ borderBottom: '1px solid #F3F4F6' }}>
            <div onClick={() => setExpandida(expandida === e.id ? null : e.id)}
              ...
              <span style={{ padding: '3px 10px', background: bgResultado(e.resultado), color: colorResultado(e.resultado), fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: '700', borderRadius: '4px' }}>
                {etiqueta(e.resultado)}
              </span>
              <span style={{ color: '#94A3B8', fontSize: 'clamp(12px, 0.90vw, 14px)' }}>{expandida === e.id ? '▲' : '▼'}</span>
            </div>
            {expandida === e.id && e.detalles.length > 0 && (
              <div style={{ padding: '0 16px 12px 42px' }}>
                {e.detalles.map((d: any, i: number) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 0', fontSize: 'clamp(12px, 0.90vw, 14px)' }}>
                    <span>{d.cumple ? '✓' : '✗'}</span>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 48px)', overflow: 'hidden', position: 'relative' as const }} onClick={() => setActiveTooltip(null)}>
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(11px, 1.3vw, 16px)', maxWidth: 'min(600px, 88vw)', maxHeight: '88vh', overflowY: 'auto', width: 'min(100%, 92vw)', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
              <div style={{ fontSize: 'clamp(14.0px, 1.21vw, 19.5px)', textAlign: 'center' as const, marginBottom: '8px' }}>🎯</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(11px, 1.3vw, 16px)', maxWidth: 'min(640px, 88vw)', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              ...
              <button onClick={() => setShowGlosario(false)} aria-label="Cerrar" style={{ background: 'none', border: 'none', fontSize: 'clamp(11.4px, 0.98vw, 15.8px)', cursor: 'pointer', color: '#94a3b8' }}>✕</button>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(11px, 1.3vw, 16px)', maxWidth: 'min(640px, 88vw)', maxHeight: '88vh', overflowY: 'auto', width: 'min(100%, 92vw)', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              ...
              <button onClick={() => setShowGuiaEdadMod40(false)} style={{ background: 'none', border: 'none', fontSize: 'clamp(11.4px, 0.98vw, 15.8px)', cursor: 'pointer', color: '#94a3b8' }}>✕</button>
            </div>
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(11px, 1.3vw, 16px)', maxWidth: 'min(600px, 88vw)', maxHeight: '88vh', overflowY: 'auto', width: 'min(100%, 92vw)', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <div>
                ...
                <p style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#94a3b8', margin: '2px 0 0' }}>Art. 167 LSS 1973 · La cuantía aumenta 1.25% por cada 52 semanas adicionales sobre 500</p>
              </div>
              <button onClick={() => setShowTooltipCuantia(false)} style={{ background: 'none', border: 'none', fontSize: 'clamp(11.4px, 0.98vw, 15.8px)', cursor: 'pointer', color: '#94a3b8' }}>✕</button>
            </div>
                ...
                <thead>
                  <tr style={{ background: AZUL }}>
                    {['Semanas cotizadas', 'Años cotizados', '% del SDI', 'Sobre pensión base'].map((h, i) => (
                      <th key={i} style={{ padding: '7px 10px', color: 'white', textAlign: i > 0 ? 'right' : 'left', fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '700' as const }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {TABLA_CUANTIA.map((row, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#F8FAFC', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '6px 10px', textAlign: 'right' as const, color: '#94a3b8', fontSize: 'clamp(12px, 0.90vw, 14px)' }}>{row.descripcion}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px', marginBottom: '12px' }}>
              <div style={{ background: '#FFF7ED', borderRadius: '10px', padding: '10px 12px', border: '1px solid #fed7aa' }}>
                <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#92400e', margin: '0 0 3px', fontWeight: '700' as const }}>CESANTÍA (60-64 años)</p>
                <p style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#92400e', margin: 0, lineHeight: 1.5 }}>Se aplica un factor reductor: 75% a los 60, 80% a los 61, 85% a los 62, 90% a los 63, 95% a los 64 años. Se requiere acreditar haber dejado de trabajar.</p>
              </div>
              <div style={{ background: '#F0FDF4', borderRadius: '10px', padding: '10px 12px', border: '1px solid #bbf7d0' }}>
                <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#15803d', margin: '0 0 3px', fontWeight: '700' as const }}>VEJEZ (65+ años)</p>
                <p style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#15803d', margin: 0, lineHeight: 1.5 }}>Factor del 100%. No requiere acreditar cesantía. Es la modalidad más conveniente si el cliente puede esperar hasta los 65 años.</p>
              </div>
            </div>
            <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#94a3b8', margin: 0, lineHeight: 1.5 }}>
      ...
      {showDetalle250 && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(13,36,64,0.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}
          ...
          <div style={{ background: 'white', borderRadius: '12px', padding: 'clamp(11.4px, 1.32vw, 15.8px)', maxWidth: 'min(720px, 88vw)', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}
            onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div>
                <h3 style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: '700' as const, color: AZUL, margin: 0 }}>Desglose completo — 250 semanas cotizadas</h3>
              ...
              <button onClick={() => setShowDetalle250(false)} style={{ background: 'none', border: 'none', fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', cursor: 'pointer', color: '#94a3b8' }}>✕</button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(11px, 0.78vw, 12.5px)' }}>
              <thead>
                <tr style={{ background: '#F4F6F9' }}>
                  {['#', 'Fecha inicio', 'Fecha fin', 'Semanas', 'SDI diario', 'SDI mensual', 'Peso'].map((h, i) => (
                    <th key={i} style={{ padding: '8px 10px', textAlign: i > 0 ? 'right' : 'center', fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '700' as const, color: '#64748b', borderBottom: '2px solid #e2e8f0' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                ...
                <tr style={{ background: '#EEF2F8', borderTop: '2px solid #e2e8f0' }}>
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'right' as const, fontWeight: '800' as const, color: NARANJA, fontSize: 'clamp(11.5px, 0.87vw, 14px)' }}>{fmtMXN2(sdiPromedio)}</td>
      ...
      {showHistorialCompleto && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(13,36,64,0.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(11px, 1.3vw, 16px)', maxWidth: 'min(680px, 88vw)', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div>
                <h3 style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: '700' as const, color: AZUL, margin: 0 }}>Historial laboral completo</h3>
              ...
              <button onClick={() => setShowHistorialCompleto(false)} style={{ background: 'none', border: 'none', fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', cursor: 'pointer', color: '#94a3b8' }}>✕</button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(11px, 0.78vw, 12.5px)' }}>
              <thead>
                <tr style={{ background: '#F4F6F9' }}>
              ...
              <tbody>
                {[...periodosCompletos].sort((a: any, b: any) => {
                  ...
                  <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '7px 10px', textAlign: 'right' as const, color: '#64748b', fontSize: 'clamp(12px, 0.90vw, 14px)' }}>{p.patron || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', backdropFilter: 'blur(2px)' }}>
          <div style={{ background: 'white', borderRadius: '12px', maxWidth: 'min(740px, 88vw)', width: 'min(100%, 94vw)', maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(13,36,64,0.28)' }}>
            ...
            <div style={{ background: AZUL, padding: '26px 28px 20px', textAlign: 'center' as const }}>
              <div style={{ fontSize: 'clamp(18.0px, 1.56vw, 25.0px)', fontWeight: '900' as const, color: 'white', letterSpacing: '-1px', fontFamily: 'Arial Black, sans-serif', marginBottom: '6px' }}>
                KSE<sup style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', verticalAlign: 'super' }}>®</sup>
              </div>
              <h2 style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: '700' as const, color: 'white', margin: '0 0 4px' }}>Calculadora de Pensión</h2>
              <p style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#93C5FD', margin: 0 }}>Completa los dos pasos para continuar</p>
            </div>
            ...
            <div style={{ padding: '20px 24px 24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ border: `2px solid ${clienteId ? VERDE : '#E5E7EB'}`, padding: '14px 16px', background: clienteId ? '#F0FDF4' : 'white', transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: clienteId ? '0' : '10px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {!clienteId && <p style={{ margin: '1px 0 0', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#94A3B8' }}>Requerido</p>}
                  </div>
                  {clienteId && (
                    <button onClick={() => setClienteId('')}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 'clamp(13px, 1.00vw, 16px)', color: '#94A3B8', padding: '0 2px' }}>✕</button>
                  )}
                </div>
                {!clienteId && (
              ...
              <div style={{ border: `1px solid ${datos.semanas_totales > 0 ? VERDE + '44' : '#E1E7F0'}`, borderRadius: '14px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', padding: 'clamp(14.0px, 1.5vw, 19.4px) clamp(14.0px, 1.67vw, 21.1px)', background: datos.semanas_totales > 0 ? '#E6F4EE' : 'white', transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: datos.semanas_totales > 0 ? '0' : '12px' }}>
                  <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: datos.semanas_totales > 0 ? VERDE : '#E1E7F0', color: datos.semanas_totales > 0 ? 'white' : '#132135', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 'clamp(12px, 0.90vw, 14px)' }}>
                    {datos.semanas_totales > 0 ? '✓' : '2'}
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: 0, fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: 700, color: '#132135' }}>
                      {datos.semanas_totales > 0 ? `Datos cargados — ${datos.semanas_totales} semanas` : 'Datos del trabajador'}
                    </p>
                    {datos.semanas_totales === 0 && !modoEntrada && <p style={{ margin: '4px 0 0', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A' }}>¿Cómo quieres ingresar los datos?</p>}
                  </div>
                  {modoEntrada && datos.semanas_totales === 0 && (
                    <button onClick={() => setModoEntrada(null)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A', padding: '6px 8px', fontFamily: 'inherit' }}>‹ Cambiar</button>
                  )}
                </div>
                  ...
                  <>
                    ...
                    {!modoEntrada && (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '12px' }}>
                        <button onClick={() => setModoEntrada('auto')}
                          ...
                          <span style={{ width: 42, height: 42, borderRadius: '50%', flexShrink: 0, background: NARANJA, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(14.0px, 1.23vw, 19.4px)', fontWeight: 700 }}>↑</span>
                          <div>
                            <div style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: 700, color: '#132135' }}>Cargar constancia del IMSS</div>
                            <div style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A', marginTop: '3px', lineHeight: 1.5 }}>Sofía lee el PDF y llena los datos sola. Es la vía recomendada: evita errores de captura.</div>
                          </div>
                          <span style={{ marginLeft: 'auto', fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', color: NARANJA, alignSelf: 'center' }}>›</span>
                        </button>
                        <button onClick={() => setModoEntrada('manual')}
                          ...
                          <span style={{ width: 42, height: 42, borderRadius: '50%', flexShrink: 0, background: '#E1E7F0', color: '#132135', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(14.0px, 1.23vw, 19.4px)', fontWeight: 700 }}>✎</span>
                          <div>
                            <div style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: 700, color: '#132135' }}>Capturar a mano</div>
                            <div style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A', marginTop: '3px', lineHeight: 1.5 }}>Para cuando no tienes el PDF o la constancia es un escaneo.</div>
                          </div>
                          <span style={{ marginLeft: 'auto', fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', color: '#C7D0DD', alignSelf: 'center' }}>›</span>
                        </button>
                      </div>
                    )}
                    ...
                    {modoEntrada === 'auto' && (
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px' }}>
                        <p style={{ margin: '0 0 6px', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A', lineHeight: 1.55 }}>
                        ...
                        {extracting && (
                          <div style={{ background: '#F5F7FA', borderRadius: '10px', padding: '14px 16px', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#132135', lineHeight: 1.55 }}>
                            Sofía está leyendo el documento. Suele tomar entre 10 y 30 segundos.
                          </div>
                        )}
                        <p style={{ margin: 0, fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#94A3B8' }}>Solo archivos PDF del portal oficial del IMSS. No funciona con fotos ni escaneos.</p>
                      </div>
                    )}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '8px' }}>
                          <div style={{ gridColumn: '1 / -1' }}>
                            <label style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '600' as const, color: '#94A3B8', display: 'block', marginBottom: '4px' }}>Nombre del trabajador</label>
                          ...
                          <div>
                            <label style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '600' as const, color: '#94A3B8', display: 'block', marginBottom: '4px' }}>Semanas cotizadas *</label>
                          ...
                          <div>
                            <label style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '600' as const, color: '#94A3B8', display: 'block', marginBottom: '4px' }}>NSS</label>
                            <input type="text" placeholder="11 dígitos"
                              onChange={e => setDatos(prev => ({ ...prev, nss: e.target.value }))}
                              style={{ width: '100%', padding: '9px 12px', border: '1px solid #E2E8F0', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', boxSizing: 'border-box' as const, fontFamily: 'inherit' }} />
                          </div>
                          <div>
                            <label style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '600' as const, color: '#94A3B8', display: 'block', marginBottom: '4px' }}>SDI actual ($/día)</label>
                        ...
                        <p style={{ margin: 0, fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#94A3B8' }}>* Las semanas son requeridas para habilitar el botón Continuar.</p>
                      </div>
                    )}
                  </>
                )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.4)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', maxWidth: 'min(580px, 88vw)', maxHeight: '88vh', overflowY: 'auto', width: 'min(100%, 92vw)', boxShadow: '0 24px 64px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <p style={{ margin: 0, fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '700' as const, color: AZUL }}>Seleccionar cliente</p>
                <p style={{ margin: '2px 0 0', fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#94A3B8' }}>Selecciona un Prospecto o cliente en Diagnóstico</p>
              </div>
              <button onClick={() => { setShowClienteModal(false); setBuscarCliente('') }}
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', color: '#94A3B8', lineHeight: 1, padding: '0 4px' }}>✕</button>
            </div>
              ...
              {clientes.filter(c => c.nombre.toLowerCase().includes(buscarCliente.toLowerCase())).map(c => (
                <button key={c.id}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#94A3B8' }}>{c.telefono ?? ''}</div>
                  </div>
                  <span style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '700' as const, padding: '3px 10px', background: c.etapa_kanban === 'diagnostico' ? '#DCFCE7' : '#EEF2F8', color: c.etapa_kanban === 'diagnostico' ? '#15803D' : AZUL, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', borderBottom: '1px solid #F3F4F6', background: 'white', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', gap: '10px', overflowX: 'auto' }}>
                <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '700' as const, color: '#1e293b', margin: 0, whiteSpace: 'nowrap' }}>
                  {TABS[tab]}
                ...
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexShrink: 0 }}>
                  <button
                    ...
                    style={{ padding: '5px 10px', background: showGuia ? '#EEF2F8' : 'white', border: '1px solid #E5E7EB', cursor: 'pointer', fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: '600' as const, color: '#334E7B', fontFamily: 'inherit' }}
            ...
            {showGuia && (
              <div style={{ position: 'absolute' as const, top: 0, right: 0, width: 'min(100%, 360px)', maxWidth: '92vw', height: '100%', background: 'white', borderLeft: '1px solid #E1E7F0', zIndex: 50, overflowY: 'auto' as const, boxShadow: '-8px 0 32px rgba(13,36,64,0.14)' }}>
                <div style={{ background: '#0D2440', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky' as const, top: 0, zIndex: 1 }}>
                  <p style={{ fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: 700, color: 'white', margin: 0 }}>Glosario</p>
                ...
                <div style={{ padding: '16px' }}>
                  {Object.entries(GLOSARIO).map(([id, g]) => (
                    <div key={id} style={{ padding: 'clamp(11px, 1.25vw, 16px) clamp(11.4px, 1.23vw, 15.8px)', marginBottom: '10px', background: '#F5F7FA', borderRadius: '12px' }}>
                      <p style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: 700, color: '#132135', margin: '0 0 6px' }}>{g.titulo}</p>
                      <p style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#132135', margin: '0 0 6px', lineHeight: 1.65 }}>{g.desc}</p>
                      {g.ejemplo && <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#66738A', margin: 0, lineHeight: 1.6 }}>{g.ejemplo}</p>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              return (
                <div className="kse-riel" style={{ background: '#F4F6F9', borderBottom: `1px solid ${BORDE}`, flexShrink: 0, position: 'relative' as const, zIndex: 20, padding: '6px 8px' }}>
                      ...
                      return (
                        <div key={gi} className="kse-riel-item" style={{ flex: 1, position: 'relative' as const }}>
                          <button
                            ...
                            {/* Ícono grande */}
                            <span style={{ fontSize: 'clamp(14.0px, 1.21vw, 19.4px)', lineHeight: 1, display: 'block' }}>
                              <i className={`ti ${g.tiIcon}`} style={{ color: activo ? 'white' : '#94A3B8', fontSize: 'clamp(14.0px, 1.21vw, 19.4px)' }} />
                            </span>
                            {/* Label */}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                              <span style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: activo ? '700' : '500', color: activo ? 'white' : '#9CA3AF', whiteSpace: 'nowrap' as const }}>{g.label}</span>
                          ...
                          {abierto && (
                            <div style={{ position: 'absolute' as const, top: 'calc(100% + 4px)', left: 0, minWidth: '210px', background: 'white', borderRadius: '10px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', border: `1px solid ${BORDE}`, zIndex: 100, overflow: 'hidden' }}>
                              <div style={{ background: AZUL, padding: '8px 14px' }}>
                                <span style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: '700', color: 'white', display: 'flex', alignItems: 'center', gap: '6px' }}>
                              ...
                              {g.tabs.map((tabIdx, si) => {
                                ...
                                return (
                                  <button key={tabIdx}
                                    ...
                                    <div style={{ width: '22px', height: '22px', borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '700', background: esCompletado ? VERDE : esCurrent ? AZUL : '#F1F5F9', color: esCompletado || esCurrent ? 'white' : '#94A3B8' }}>
                                      {esCompletado ? '✓' : si + 1}
                                    </div>
                                    <div style={{ flex: 1 }}>
                                      ...
                                      {esCurrent && <div style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: AZUL, marginTop: '1px' }}>← Estás aquí</div>}
                                    </div>
                  ...
                  <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px', alignItems: 'center' }}>
                    <span style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#94A3B8', whiteSpace: 'nowrap' as const }}>Opciones:</span>
                    <button onClick={() => setTab(12)}
                      style={{ padding: '6px 12px', borderRadius: '10px', border: `1px solid ${tab === 12 ? '#0891B2' : '#E2E8F0'}`, background: tab === 12 ? '#ECFEFF' : 'white', color: tab === 12 ? '#0891B2' : '#64748B', fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: '600' as const, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' as const }}>
            ...
            <div style={{ display: 'flex', background: 'white', borderBottom: `1px solid ${BORDE}`, flexShrink: 0, overflowX: 'auto' }}>
              {[
                ...
                return (
                  ...
                  <div key={i} style={{ flex: '0 0 auto', padding: 'clamp(10px, 1.2vw, 14px) clamp(11.4px, 1.32vw, 15.8px)', borderRight: `1px solid ${BORDE}`, background: 'white', minWidth: '132px' }}>
                    ...
                    <div style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: 700, color: '#132135', whiteSpace: 'nowrap' as const, fontVariantNumeric: 'tabular-nums' as const }}>{k.value}</div>
                  </div>
                )
              })}
            </div>
{/* Contenido de la pestaña actual */}
            <div style={{ flex: 1, overflowY: 'auto', padding: 'clamp(10px, 1.2vw, 14px) clamp(11.4px, 1.23vw, 15.8px)', background: '#F5F7FA', fontSize: 'clamp(12px, 0.90vw, 14px)', minWidth: 0, position: 'relative' as const }}>
            ...
            {tab === -1 && (
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 'clamp(9px, 1.1vw, 14px)' }}>
                ...
                {(escenarios[0]?.pension_mensual || sdiPromedio > 0) && (
                  <section style={{ position: 'relative' as const, overflow: 'hidden', borderRadius: '14px', background: 'linear-gradient(118deg, #0D2440 0%, #14375F 60%, #245287 100%)' }}>
                    ...
                    <div style={{ position: 'relative' as const, padding: '14px 18px 10px' }}>
                      ...
                      <p style={{ fontSize: 'clamp(19.9px, 2.11vw, 27.7px)', fontWeight: 800, color: 'white', margin: '8px 0 0', lineHeight: 1, letterSpacing: '-.035em', whiteSpace: 'nowrap' as const, fontVariantNumeric: 'tabular-nums' as const }}>
                    ...
                    <div style={{ position: 'relative' as const, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '1px', background: 'rgba(255,255,255,.11)' }}>
                      {[
                        ...
                        <div key={i} style={{ background: '#0D2440', padding: '13px 18px' }}>
                          ...
                          <p style={{ fontSize: 'clamp(14.0px, 1.19vw, 19.4px)', fontWeight: 700, color: k.color, margin: '3px 0 0', whiteSpace: 'nowrap' as const, fontVariantNumeric: 'tabular-nums' as const }}>{k.value}</p>
                ...
                <div style={{ display: 'flex', gap: 'clamp(9px, 1.1vw, 14px)', padding: '10px 15px', background: '#FFFFFF', border: '1px solid #E1E7F0', borderRadius: '12px', flexWrap: 'wrap' as const, alignItems: 'center' }}>
                  ...
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                      ...
                      <span style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#132135' }}>{item.label}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #E1E7F0', padding: 'clamp(14.0px, 1.5vw, 19.4px) clamp(14.0px, 1.67vw, 21.1px)', boxShadow: '0 1px 3px rgba(19,33,53,0.06)' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '12px', marginBottom: '16px' }}>
                    <span style={{ fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: 700, color: '#132135' }}>Identificación</span>
                    <span style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A' }}>{clientes.find(c => c.id === clienteId)?.nombre || 'Sin cliente'}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '8px' }}>
                    {[
                      ...
                      <div key={i}>
                        <div style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A', marginBottom: '6px' }}>{k.label}</div>
                        <div style={{ padding: '12px 14px', background: k.value ? '#F5F7FA' : 'white', border: '1px solid #E1E7F0', borderRadius: '10px', fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: k.highlight ? 700 : 500, color: k.value ? '#132135' : '#66738A', fontStyle: k.value ? 'normal' : 'italic' }}>
                          {k.value || '—'}
                        </div>
                      </div>
                    ))}
                ...
                <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #E1E7F0', padding: 'clamp(14.0px, 1.5vw, 19.4px) clamp(14.0px, 1.67vw, 21.1px)', boxShadow: '0 1px 3px rgba(19,33,53,0.06)' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '12px', marginBottom: '16px' }}>
                    <span style={{ fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: 700, color: '#132135' }}>Cotización</span>
                    <span style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', fontWeight: 600, color: conservacion.vigente ? VERDE : '#DC2626' }}>{datos.semanas_totales ? (conservacion.vigente ? '✓ Vigente' : '✕ Vencida') : '—'}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px', marginBottom: '10px' }}>
                    {[
                      ...
                      <div key={i} style={{ textAlign: 'center' as const, padding: '10px 8px', background: k.bg, border: `1px solid ${k.border}`, borderRadius: '10px' }}>
                        <div style={{ fontSize: 'clamp(14.0px, 1.19vw, 19.4px)', fontWeight: '800' as const, color: k.color, lineHeight: 1 }}>{k.value}</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px' }}>
                  ...
                  <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #E1E7F0', padding: '10px 15px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                    <span style={{ fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: 700, background: '#F0F7F4', color: VERDE, padding: '3px 8px', borderRadius: '4px', display: 'inline-block', marginBottom: '12px' }}>Familia y beneficiarios</span>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '6px' }}>
                      {[
                        ...
                        <div key={i} style={{ textAlign: 'center' as const, padding: '10px 6px', background: ok ? '#F0F7F4' : '#F8FAFC', border: `1px solid ${ok ? '#86EFAC' : '#E2E8F0'}`, borderRadius: '10px' }}>
                          <div style={{ fontSize: 'clamp(14.0px, 1.21vw, 19.4px)', fontWeight: '800' as const, color: ok ? VERDE : '#9CA3AF' }}>{value}</div>
                  ...
                  <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #E1E7F0', padding: '10px 15px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                    <span style={{ fontSize: 'clamp(11.4px, 1.01vw, 15.8px)', fontWeight: 700, background: '#F5F3FF', color: '#7C3AED', padding: '3px 8px', borderRadius: '4px', display: 'inline-block', marginBottom: '12px' }}>% Pensión por edad (Ley 73)</span>
                    <table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: 'clamp(12px, 0.90vw, 14px)' }}>
                      ...
                      <tbody>
                        {[[60,'75%','Cesantía'],[61,'80%','Cesantía'],[62,'85%','Cesantía'],[63,'90%','Cesantía'],[64,'95%','Cesantía']].map(([edad, pct, tipo], i) => {
                          ...
                          return (
                            <tr key={i} style={{ background: isActive ? '#EEF2F8' : i % 2 === 0 ? 'white' : '#F9FAFB', borderBottom: '1px solid #F3F4F6' }}>
                              ...
                              <td style={{ padding: '6px 8px', color: '#64748B', fontSize: 'clamp(11.5px, 0.87vw, 14px)' }}>{tipo as string}</td>
                            </tr>
                          )
                        })}
                        <tr style={{ background: VERDE }}>
                          ...
                          <td style={{ padding: '6px 8px', color: 'white', fontSize: 'clamp(11.5px, 0.87vw, 14px)', fontWeight: '600' as const }}>Vejez (IDEL)</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
                  {escsConMod40.map((esc: any, i: number) => (
                    <div key={i} style={{ background: esc.recomendado ? VERDE : 'white', border: esc.recomendado ? 'none' : '1.5px solid #E2E8F0', borderRadius: '10px', padding: '14px', textAlign: 'center' as const, boxShadow: esc.recomendado ? '0 4px 12px rgba(46,125,90,0.2)' : '0 1px 3px rgba(0,0,0,0.06)' }}>
                      {esc.recomendado && <div style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: 'rgba(255,255,255,0.8)', marginBottom: '6px', fontWeight: '600' as const }}>⭐ RECOMENDADO</div>}
                      <div style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: esc.recomendado ? 'rgba(255,255,255,0.7)' : '#94A3B8', marginBottom: '4px' }}>{esc.mod40_meses} meses Mod. 40</div>
                      <div style={{ fontSize: 'clamp(14.0px, 1.19vw, 19.4px)', fontWeight: '800' as const, color: esc.recomendado ? 'white' : AZUL, letterSpacing: '-0.5px' }}>
                        {fmtMXN(esc.pension_mensual)}
                      </div>
                      <div style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: esc.recomendado ? 'rgba(255,255,255,0.7)' : '#64748B', marginTop: '2px' }}>mensual</div>
                      ...
                      <div style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: esc.recomendado ? 'rgba(255,255,255,0.6)' : '#94A3B8' }}>inversión total</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ background: '#0891B2', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <p style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#BAE6FD', margin: '0 0 3px', fontWeight: '600' }}>Cotización Voluntaria</p>
                  <p style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: '800', color: 'white', margin: 0 }}>Modalidad 10 — Conservación de Derechos</p>
                </div>
                {escM10 && (
                  <div style={{ textAlign: 'right' as const }}>
                    <div style={{ fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#BAE6FD', marginBottom: '2px' }}>Pensión mejorada</div>
                    <div style={{ fontSize: 'clamp(14.5px, 1.27vw, 20.2px)', fontWeight: '900', color: 'white', letterSpacing: '-1px' }}>{fmtMXN2(escM10.pension_mensual)}</div>
                  </div>
                )}
              </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '14px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {[
                      ...
                      <div key={i} style={{ display: 'flex', gap: '14px', padding: 'clamp(11px, 1.25vw, 16px) clamp(11.4px, 1.23vw, 15.8px)', background: '#F5F7FA', borderRadius: '12px' }}>
                        <span style={{ width: 6, height: 6, borderRadius: 999, background: '#0891B2', flexShrink: 0, marginTop: 8 }} />
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 'clamp(13px, 1.00vw, 16px)', fontWeight: 600, color: '#132135' }}>{title}</div>
                          <div style={{ fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A', marginTop: 2, lineHeight: 1.55 }}>{desc}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <>
                  <section style={{ position: 'relative' as const, overflow: 'hidden', borderRadius: '14px', background: 'linear-gradient(118deg, #0D2440 0%, #14375F 60%, #0891B2 100%)' }}>
                    ...
                    <div style={{ position: 'relative' as const, padding: '14px 18px 10px' }}>
                      ...
                      <p style={{ fontSize: 'clamp(27.6px, 3.04vw, 38.3px)', fontWeight: 800, color: 'white', margin: '8px 0 0', lineHeight: 1, letterSpacing: '-.035em', whiteSpace: 'nowrap' as const, fontVariantNumeric: 'tabular-nums' as const }}>
                    ...
                    <div style={{ position: 'relative' as const, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '1px', background: 'rgba(255,255,255,.11)' }}>
                      {[
                        ...
                        <div key={i} style={{ background: '#0D2440', padding: '13px 18px' }}>
                          <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: 'rgba(255,255,255,.56)', margin: 0 }}>{k.label}</p>
                          <p style={{ fontSize: 'clamp(14.5px, 1.26vw, 20.2px)', fontWeight: 700, color: k.color, margin: '3px 0 0', whiteSpace: 'nowrap' as const, fontVariantNumeric: 'tabular-nums' as const }}>{k.value}</p>
                          <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: 'rgba(255,255,255,.44)', margin: '2px 0 0' }}>{k.sub}</p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      ...
                      <div style={DS.card}>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(11px, 0.78vw, 12.5px)' }}>
                          <thead>
                            <tr style={{ background: '#F4F6F9' }}>
                              <th style={{ padding: '7px 10px', textAlign: 'left' as const, fontSize: 'clamp(12px, 0.90vw, 14px)', color: '#66738A', fontWeight: '700' }}>Concepto</th>
                              <th style={{ padding: '7px 10px', textAlign: 'center' as const, fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#0891B2', fontWeight: '700' }}>Mod 10</th>
                              <th style={{ padding: '7px 10px', textAlign: 'center' as const, fontSize: 'clamp(11.5px, 0.87vw, 14px)', color: '#334E7B', fontWeight: '700' }}>Mod 40</th>
                            </tr>
                          </thead>
                          <tbody>
                            {[
              ...
              {!escM10 && (
                <div style={DS.card}>
                  <div style={{ textAlign: 'center' as const, padding: '24px 18px', color: '#94A3B8' }}>
                    <div style={{ fontSize: 'clamp(22.8px, 1.98vw, 31.7px)', marginBottom: '12px' }}>📋</div>
