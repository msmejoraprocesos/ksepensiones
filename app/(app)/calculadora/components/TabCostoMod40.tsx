...
export default function TabCostoMod40({ escenarios, sys, getMod40Pct, setTab }: Props) {
  ...
  if (!escRec || escRec.mod40_meses === 0) return (
    <div style={{ textAlign: 'center', padding: '60px', color: K.muted }}>
      ...
      <p style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)' }}>Configura el Salario Mod. 40 para ver el costo</p>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 1.6vw, 20px)' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', borderRadius: '18px', background: `linear-gradient(118deg, ${K.navy900} 0%, ${K.navy800} 60%, ${K.navy600} 100%)` }}>
        ...
        <div style={{ position: 'relative', padding: '28px 34px 22px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '16px', flexWrap: 'wrap', marginTop: '8px' }}>
            ...
            <span style={{ fontSize: 'clamp(14px, 1.06vw, 17px)', color: 'rgba(255,255,255,.7)' }}>de inversión neta</span>
          </div>
      ...
      <div className="kse-2col" style={{ display: 'grid', gridTemplateColumns: 'minmax(340px, 1.6fr) minmax(280px, 1fr)', gap: 'clamp(12px, 1.6vw, 20px)' }}>
        ...
        <div style={{ background: K.card, border: `1px solid ${K.line}`, borderRadius: '14px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', padding: 'clamp(14px, 1.9vw, 24px)' }}>
          <p style={{ fontSize: 'clamp(16px, 1.25vw, 20px)', fontWeight: 700, color: K.ink, margin: '0 0 4px' }}>Cuánto se paga cada año</p>
            ...
            <thead>
              <tr style={{ borderBottom: `1px solid ${K.line}` }}>
                {['Año', 'Meses', 'Tasa', 'Cuota mensual', 'Subtotal'].map((h, i) => (
                  <th key={h} style={{ padding: '10px 12px', fontSize: 'clamp(11px, 0.80vw, 13px)', color: K.muted, fontWeight: 500, textAlign: i ? 'right' : 'left' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
          ...
          <div style={{ marginTop: '14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', background: K.navy900, borderRadius: '10px', padding: '16px 20px' }}>
            <span style={{ fontSize: 'clamp(14px, 1.06vw, 17px)', color: 'rgba(255,255,255,.78)' }}>Total de {totalMeses} meses</span>
            <span style={{ fontSize: 'clamp(18px, 1.50vw, 24px)', fontWeight: 700, color: 'white', ...nw, ...num }}>{fmtMXN2(escRec.costo_total)}</span>
          </div>

          {Math.abs(sumaFilas - escRec.costo_total) > 1 && (
            <p style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', color: K.muted, margin: '10px 0 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 1.6vw, 20px)' }}>
          ...
          <div style={{ background: K.card, border: `1px solid ${K.line}`, borderRadius: '14px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', padding: 'clamp(14px, 1.9vw, 24px)' }}>
            <p style={{ fontSize: 'clamp(16px, 1.25vw, 20px)', fontWeight: 700, color: K.ink, margin: '0 0 14px' }}>Flujo de caja</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', padding: '12px 0', borderTop: i ? `1px solid ${K.line}` : 'none' }}>
                ...
                <span style={{ fontSize: 'clamp(14px, 1.06vw, 17px)', fontWeight: 700, color: r.color, ...nw, ...num }}>{r.value}</span>
              </div>
            ))}
            <div style={{ marginTop: '10px', background: K.orangeSoft, borderRadius: '10px', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
              <span style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', color: K.ink, fontWeight: 600 }}>Inversión neta final</span>
              <span style={{ fontSize: 'clamp(16px, 1.25vw, 20px)', fontWeight: 800, color: K.orange, ...nw, ...num }}>{fmtMXN2(escRec.inversion_neta)}</span>
            </div>
          </div>

          <div style={{ background: K.card, border: `1px solid ${K.line}`, borderRadius: '14px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', padding: 'clamp(14px, 1.9vw, 24px)' }}>
            <p style={{ fontSize: 'clamp(16px, 1.25vw, 20px)', fontWeight: 700, color: K.ink, margin: '0 0 14px' }}>Rentabilidad</p>
            {[
            ...
            <div style={{ marginTop: '14px', background: t.bg, borderRadius: '10px', padding: '14px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
                <p style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: 700, color: t.color, margin: 0 }}>{t.label}</p>
                <GuiaTermometro meses={escRec.roi} tam="md" />
              </div>
              <p style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', color: K.ink, margin: '4px 0 0', lineHeight: 1.55 }}>{t.explica}</p>
            </div>
          </div>
        </div>
      </div>
