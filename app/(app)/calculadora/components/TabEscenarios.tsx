            ...
            return (
              <button key={i} onClick={() => setSel(i)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                  <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: 700, color: c, ...nw }}>
              ...
              return (
                <button key={i} onClick={() => { if (f.idx >= 0) setSel(f.idx) }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <span style={{ width: '168px', flexShrink: 0, fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: on ? K.ink : K.muted, fontWeight: on || f.rec ? 700 : 500, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', ...nw }}>
                      {f.rec && '* '}{f.label}
                    </span>
                    <span style={{ flex: 1, height: '30px', background: K.paper, borderRadius: '6px', position: 'relative', overflow: 'hidden' }}>
                      <span style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: anim ? `${pct}%` : '0%', background: f.color, borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: '10px', opacity: f.idx >= 0 && !on ? 0.45 : 1, transition: 'width .9s cubic-bezier(.22,1,.36,1), opacity .25s' }}>
                        <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: 700, color: 'white', ...nw, ...num }}>{fmtMXN2(f.value)}</span>
                      </span>
                    </span>
                  </div>
                </button>
          ...
          <div className="kse-detalle" style={{ background: K.card, border: `1px solid ${K.line}`, borderRadius: '14px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', overflow: 'hidden', alignSelf: 'start' }}>
            ...
            <div style={{ background: K.paper, borderTop: `1px solid ${K.line}`, padding: '12px 16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', marginTop: '10px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: K.muted }}>
                  <strong style={{ color: K.ink }}>{escSel.roi} meses</strong> para recuperar
                </span>
                <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: K.green, fontWeight: 700, ...nw }}>
              ...
              <div style={{ marginTop: '12px', background: t.bg, borderRadius: '9px', padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
                  <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: 700, color: t.color, margin: 0 }}>{t.label}</p>
                  <GuiaTermometro meses={escSel.roi} />
                </div>
                <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: K.ink, margin: '4px 0 0', lineHeight: 1.5 }}>{t.explica}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(9.6px, 0.79vw, 12.3px)' }}>
              <thead>
                <tr style={{ background: K.navy900 }}>
                  ...
                  <th style={{ padding: '13px 16px', color: 'rgba(255,255,255,0.6)', textAlign: 'right', fontWeight: 600, fontSize: 'clamp(11px, 0.78vw, 12.5px)', ...nw }}>Sin Mod. 40</th>
                  {escsConMod40.slice(0, 6).map((e, i) => (
                    <th key={i} onClick={() => setSel(i)} style={{ padding: '13px 16px', color: i === idxSel ? K.gold : 'white', textAlign: 'right', fontWeight: 700, fontSize: 'clamp(11px, 0.78vw, 12.5px)', cursor: 'pointer', ...nw }}>
