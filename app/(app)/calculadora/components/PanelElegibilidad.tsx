    ...
    <div style={{ background: K.card, borderRadius: '14px', border: `1px solid ${K.line}`, boxShadow: '0 1px 3px rgba(19,33,53,0.06)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: 'clamp(12px, 1.6vw, 20px) clamp(14px, 1.9vw, 24px)', background: r.mod40_viable ? K.greenSoft : K.redSoft, borderBottom: `1px solid ${K.line}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
          ...
          <div style={{ minWidth: 0 }}>
            <p style={{ fontSize: 'clamp(16px, 1.25vw, 20px)', fontWeight: 700, color: K.ink, margin: 0 }}>
              {r.mod40_viable ? 'Puede acceder a Modalidad 40' : 'No puede acceder a Modalidad 40'}
            </p>
            <p style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', color: K.muted, margin: '3px 0 0' }}>
              {r.mod40_viable
          ...
          {r.mod10_recomendada && (
            <span style={{ marginLeft: 'auto', background: K.cyan, color: 'white', fontSize: 'clamp(11px, 0.80vw, 13px)', fontWeight: 700, padding: '7px 13px', borderRadius: 999, ...nw }}>
      ...
      <div style={{ padding: 'clamp(12px, 1.6vw, 20px) clamp(14px, 1.9vw, 24px)', borderBottom: `1px solid ${K.line}` }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <div>
            <label style={{ display: 'block', fontSize: 'clamp(12.5px, 0.94vw, 15px)', color: K.muted, marginBottom: '6px' }}>
            ...
            <input
              ...
              style={campo}
            />
            <p style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', color: K.muted, margin: '5px 0 0' }}>
            ...
            <div key={i} style={{ display: 'flex', gap: '13px', padding: '14px 16px', marginBottom: '8px', background: c.bg, border: `1px solid ${c.borde}`, borderRadius: '10px' }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 'clamp(14px, 1.06vw, 17px)', fontWeight: 600, color: K.ink }}>{h.titulo}</span>
                  <span style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', fontWeight: 700, color: c.fg, ...nw }}>{c.titulo}</span>
                  {h.fundamento && (
                    <span style={{ marginLeft: 'auto', fontSize: 'clamp(11px, 0.80vw, 13px)', color: K.muted, ...nw }}>{h.fundamento}</span>
                  )}
                </div>
                <p style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', color: K.ink, margin: '4px 0 0', lineHeight: 1.6 }}>{h.detalle}</p>
              </div>
            </div>
          )
        })}
