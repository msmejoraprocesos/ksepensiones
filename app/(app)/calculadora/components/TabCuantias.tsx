...
export default function TabCuantias({ res, sdiPromedio, datos, setTab, Tip }: Props) {
  ...
  if (sdiPromedio <= 0) return (
    <div style={{ textAlign: 'center', padding: '30px 18px', color: K.muted }}>
      <i className="ti ti-file-alert" style={{ fontSize: 'clamp(17.8px, 1.42vw, 22.8px)', display: 'block', marginBottom: '12px' }} />
      <p style={{ fontSize: 'clamp(9.6px, 0.79vw, 12.3px)' }}>Carga la constancia IMSS para ver la pensión actual</p>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(9.0px, 0.97vw, 12.3px)' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', borderRadius: '14px', background: `linear-gradient(118deg, ${K.navy900} 0%, ${K.navy800} 60%, ${K.navy600} 100%)` }}>
        ...
        <div style={{ position: 'relative', padding: '14px 18px 10px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', flexWrap: 'wrap', marginTop: '8px' }}>
            ...
            {res.pmg_aplica && (
              <span style={{ background: 'rgba(255,255,255,.13)', color: K.gold, fontSize: 'clamp(9.6px, 0.79vw, 12.3px)', fontWeight: 700, padding: '7px 13px', borderRadius: 999, border: `1px solid ${K.gold}55`, ...nw }}>
      ...
      <div className="kse-2col" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 1.15fr) minmax(280px, 1fr)', gap: 'clamp(9.0px, 0.97vw, 12.3px)' }}>
        ...
        <div style={{ background: K.card, border: `1px solid ${K.line}`, borderRadius: '14px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', padding: 'clamp(11.0px, 1.15vw, 14.1px)' }}>
          <p style={{ fontSize: 'clamp(10.8px, 0.89vw, 13.9px)', fontWeight: 700, color: K.ink, margin: '0 0 18px' }}>De qué se compone la pensión anual</p>
            ...
            {componentes.map((c, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px', background: K.paper, borderRadius: '10px', padding: '14px 16px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 'clamp(11.0px, 0.88vw, 14.1px)', color: K.ink, fontWeight: 600, margin: 0 }}>{c.label}</p>
                  <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: K.muted, margin: '2px 0 0' }}>{c.nota}</p>
                </div>
                <span style={{ fontSize: 'clamp(11.0px, 0.88vw, 14.1px)', fontWeight: 700, color: K.ink, ...nw, ...num }}>{fmtMXN2(c.val)}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: '14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', background: K.navy900, borderRadius: '10px', padding: '16px 20px' }}>
            <span style={{ fontSize: 'clamp(11.0px, 0.88vw, 14.1px)', color: 'rgba(255,255,255,.78)' }}>Total anual</span>
            <span style={{ fontSize: 'clamp(13.3px, 1.05vw, 17.1px)', fontWeight: 700, color: 'white', ...nw, ...num }}>{fmtMXN2(res.pensionAnual)}</span>
          </div>
        </div>

        {/* ── Factores ──────────────────────────────────────────── */}
        <div style={{ background: K.card, border: `1px solid ${K.line}`, borderRadius: '14px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', padding: 'clamp(11.0px, 1.15vw, 14.1px)' }}>
          <p style={{ fontSize: 'clamp(10.8px, 0.89vw, 13.9px)', fontWeight: 700, color: K.ink, margin: '0 0 14px' }}>Factores del cálculo</p>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', padding: '12px 0', borderTop: i ? `1px solid ${K.line}` : 'none' }}>
              <span style={{ fontSize: 'clamp(9.6px, 0.79vw, 12.3px)', color: K.muted }}>{l}</span>
              <span style={{ fontSize: 'clamp(11.0px, 0.88vw, 14.1px)', fontWeight: 700, color: K.ink, ...nw, ...num }}>{v}</span>
            </div>
          ))}

          {res.pmg_aplica ? (
            <div style={{ marginTop: '16px', background: K.redSoft, border: `1px solid ${K.red}44`, borderRadius: '10px', padding: '14px 16px' }}>
              <p style={{ fontSize: 'clamp(9.6px, 0.79vw, 12.3px)', color: K.ink, margin: 0, lineHeight: 1.6 }}>
            ...
            <thead>
              <tr style={{ borderTop: `1px solid ${K.line}`, borderBottom: `1px solid ${K.line}` }}>
                {['Edad', 'Tipo', 'Factor', 'Mensual', 'Anual', 'Aguinaldo'].map((hd, i) => (
                  <th key={hd} style={{ padding: '11px 14px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: K.muted, fontWeight: 500, textAlign: i < 2 ? 'left' : 'right', ...nw }}>{hd}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button onClick={() => setTab(2)}
          ...
          Salario Mod. 40 <i className="ti ti-arrow-right" style={{ fontSize: 'clamp(11.0px, 0.88vw, 14.1px)' }} />
        </button>
      </div>
    </div>
  )
