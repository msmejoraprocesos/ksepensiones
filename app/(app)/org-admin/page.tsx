          ...
          pendientes.map(s => (
            <div key={s.id} style={{ background: 'white', border: '1px solid #E1E7F0', borderRadius: '10px', padding: '14px 16px', marginBottom: '10px' }}>
              ...
              <input placeholder="Nota para el rechazo (opcional)"
                ...
                style={{ width: '100%', padding: '7px 10px', border: '1px solid #E5E7EB', fontSize: '15px', borderRadius: '10px', fontFamily: 'inherit', boxSizing: 'border-box' as const, marginBottom: '10px' }} />

              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => resolver(s.id, 'rechazada', s.cliente_id, s.asesor_destino_id)}
                  disabled={procesando === s.id}
                  style={{ flex: 1, padding: '9px', background: '#FEF2F2', color: ROJO, border: `1px solid ${ROJO}`, fontSize: '15px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px', opacity: procesando === s.id ? 0.6 : 1 }}>
                  ❌ Rechazar
                </button>
                <button onClick={() => resolver(s.id, 'aprobada', s.cliente_id, s.asesor_destino_id)}
                  disabled={procesando === s.id}
                  style={{ flex: 2, padding: '9px', background: VERDE, color: 'white', border: 'none', fontSize: '15px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px', opacity: procesando === s.id ? 0.6 : 1 }}>
          ...
          {resueltas.slice(0, 10).map(s => {
            ...
            return (
              <div key={s.id} style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: '10px', padding: '10px 14px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <span style={{ padding: '3px 10px', background: badge.bg, color: badge.color, fontSize: '15px', fontWeight: '700', borderRadius: '10px', whiteSpace: 'nowrap' as const }}>
                  {badge.label}
                </span>
              </div>
            )
