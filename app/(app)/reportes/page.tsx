        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto auto', gap: '12px', alignItems: 'flex-end' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' as const }}>
              {REPORTES.map(r => (
                <button key={r.id} onClick={() => { setTipoReporte(r.id as Reporte); setDatos([]) }}
                  style={{ padding: '6px 12px', background: tipoReporte === r.id ? AZUL : 'white', color: tipoReporte === r.id ? 'white' : '#374151', border: `1px solid ${tipoReporte === r.id ? AZUL : '#E5E7EB'}`, fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: tipoReporte === r.id ? '700' : '400', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px' }}>
                  {r.label}
                </button>
              ))}
            </div>
          ...
          <div style={{ gridColumn: tipoReporte === 'cartera' ? '2 / -1' : 'auto', display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
            <button onClick={cargarDatos} disabled={cargando || !userId}
              style={{ padding: '8px 16px', background: AZUL, color: 'white', border: 'none', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px', opacity: cargando ? 0.7 : 1, whiteSpace: 'nowrap' as const }}>
      ...
      {datos.length > 0 && (
        <>
          ...
          <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: '10px', padding: '12px 16px', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#374151', margin: 0, fontWeight: '600' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={exportarExcel} disabled={generando === 'excel'}
                style={{ padding: '7px 14px', background: '#16A34A', color: 'white', border: 'none', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px', opacity: generando === 'excel' ? 0.7 : 1 }}>
                {generando === 'excel' ? 'Generando...' : '📊 Exportar Excel'}
              </button>
              <button onClick={exportarPDF} disabled={generando === 'pdf'}
                style={{ padding: '7px 14px', background: NARANJA, color: 'white', border: 'none', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px', opacity: generando === 'pdf' ? 0.7 : 1 }}>
          ...
          <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: '10px', overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' as const }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: 'clamp(11px, 0.78vw, 12.5px)' }}>
                <thead>
            ...
            {datos.length > 50 && (
              <div style={{ padding: '10px 16px', background: '#F8FAFC', borderTop: '1px solid #E5E7EB', fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#6B7280', textAlign: 'center' as const }}>
