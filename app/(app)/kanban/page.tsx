    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 56px)', background: '#F4F6F9' }}>
      ...
      <div style={{ position: 'sticky' as const, top: 0, zIndex: 10, background: 'white', borderBottom: '1px solid #e2e8f0', padding: 'clamp(12px, 1.6vw, 20px) clamp(14px, 1.9vw, 24px)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <button onClick={() => setShowNuevo(true)}
          style={{ background: AZUL, color: 'white', border: 'none', borderRadius: '10px', padding: '8px 16px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer', flexShrink: 0 }}>
          + Nuevo cliente
        </button>
      </div>
                      ...
                      <div key={cliente.id}
                        ...
                        {cliente.servicio_contratado && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                            <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', padding: '1px 6px', borderRadius: '10px', background: col.bg, color: col.color, fontWeight: '700', border: `1px solid ${col.color}30` }}>
                              {cliente.servicio_contratado}
                            </span>
                          </div>
                        )}
                        ...
                        {cliente.estatus_pago && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                            ...
                            <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: PAGO_COLOR[cliente.estatus_pago] ?? '#94a3b8', fontWeight: '600' }}>{cliente.estatus_pago}</span>
                          </div>
                        )}

                        {/* Último contacto */}
                        <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8', marginTop: '4px' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(16px, 2.2vw, 28px)', width: 'min(100%, 480px)', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <a href={`/clientes`} style={{ flex: 1, padding: '9px', background: '#F1F5F9', color: '#64748b', border: 'none', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer', textAlign: 'center', textDecoration: 'none' }}>
                Ver expediente
              </a>
              <a href={`/calculadora?cliente=${modal.cliente.id}`} style={{ flex: 1, padding: '9px', background: AZUL, color: 'white', border: 'none', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer', textAlign: 'center', textDecoration: 'none' }}>
                Abrir calculadora
              </a>
            </div>
          </div>
