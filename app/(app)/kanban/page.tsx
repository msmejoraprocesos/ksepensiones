...
export default function KanbanPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 56px)', background: '#F4F6F9' }}>
      ...
      <div style={{ position: 'sticky' as const, top: 0, zIndex: 10, background: 'white', borderBottom: '1px solid #e2e8f0', padding: 'clamp(12px, 1.6vw, 20px) clamp(14px, 1.9vw, 24px)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '16px' }}>
        <h1 style={{ color: AZUL, fontSize: 'clamp(19px, 1.62vw, 26px)', fontWeight: 700, margin: 0 }}>Pipeline de Clientes</h1>
        ...
        <div style={{ display: 'flex', gap: '12px', flex: 1 }}>
          {[
            ...
            <div key={i} style={{ background: '#F4F6F9', borderRadius: '10px', padding: '5px 12px', border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', color: '#94a3b8' }}>{k.label}</div>
        ...
        <button onClick={() => setShowNuevo(true)}
          style={{ background: AZUL, color: 'white', border: 'none', borderRadius: '10px', padding: '8px 16px', fontSize: 'clamp(11px, 0.80vw, 13px)', fontWeight: '700', cursor: 'pointer', flexShrink: 0 }}>
          + Nuevo cliente
        </button>
      </div>
                ...
                <div key={col.id}
                  ...
                  <div style={{ position: 'sticky' as const, top: '65px', zIndex: 2, background: colBg, borderRadius: '12px 12px 0 0', padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      <span style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', color: col.cierre ? col.color : '#374151' }}>{col.label}</span>
                    </div>
                    <span style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', background: col.bg, color: col.color, padding: '1px 7px', borderRadius: '10px', border: `1px solid ${col.color}30` }}>
                      {cards.length}
                    </span>
                  </div>
                  ...
                  <div style={{ padding: '0 8px 8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {cards.map(cliente => (
                      <div key={cliente.id}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
                          <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: AZUL, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', flexShrink: 0 }}>
                            {cliente.nombre.charAt(0).toUpperCase()}
                          </div>
                          <span style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', fontWeight: '700', color: '#1e293b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cliente.nombre}</span>
                        </div>

                        {/* Servicio */}
                        {cliente.servicio_contratado && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                            <span style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', padding: '1px 6px', borderRadius: '10px', background: col.bg, color: col.color, fontWeight: '700', border: `1px solid ${col.color}30` }}>
                              {cliente.servicio_contratado}
                            </span>
                          </div>
                        )}
                        ...
                        {cliente.monto_acordado && (
                          <div style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', color: AZUL, fontWeight: '700', marginBottom: '4px' }}>
                        ...
                        {cliente.estatus_pago && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                            ...
                            <span style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', color: PAGO_COLOR[cliente.estatus_pago] ?? '#94a3b8', fontWeight: '600' }}>{cliente.estatus_pago}</span>
                          </div>
                        )}

                        {/* Último contacto */}
                        <div style={{ fontSize: 'clamp(11px, 0.80vw, 13px)', color: '#94a3b8', marginTop: '4px' }}>
                    ...
                    {/* Drop zone vacía */}
                    {cards.length === 0 && (
                      <div style={{ border: `2px dashed ${isDragOver ? col.color : '#e2e8f0'}`, borderRadius: '10px', padding: '20px 8px', textAlign: 'center', color: isDragOver ? col.color : '#cbd5e1', fontSize: 'clamp(12.5px, 0.94vw, 15px)', transition: 'all 0.15s' }}>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(16px, 2.2vw, 28px)', width: 'min(100%, 480px)', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            {/* Header modal */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', color: '#94a3b8' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', color: '#374151', marginBottom: '6px' }}>Etapa en el pipeline</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {COLUMNAS.map(col => (
                  <button key={col.id} onClick={() => moverCliente(modal.cliente.id, col.id)}
                    style={{ padding: '5px 10px', borderRadius: '10px', border: `1.5px solid ${modal.cliente.etapa_kanban === col.id ? col.color : '#e2e8f0'}`, background: modal.cliente.etapa_kanban === col.id ? col.bg : 'white', color: modal.cliente.etapa_kanban === col.id ? col.color : '#64748b', fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '600', cursor: 'pointer' }}>
                    {col.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px', marginBottom: '12px' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', color: '#374151', marginBottom: '5px' }}>Estatus de pago</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', color: '#374151', marginBottom: '5px' }}>Monto cobrado ($)</label>
            ...
            <div style={{ background: '#F4F6F9', borderRadius: '10px', padding: '12px', marginBottom: '14px' }}>
              {[
                ...
                <div key={i} style={{ fontSize: 'clamp(12.5px, 0.94vw, 15px)', color: '#64748b', marginBottom: i < 2 ? '4px' : '0', display: 'flex', gap: '6px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <a href={`/clientes`} style={{ flex: 1, padding: '9px', background: '#F1F5F9', color: '#64748b', border: 'none', borderRadius: '10px', fontSize: 'clamp(11px, 0.80vw, 13px)', fontWeight: '600', cursor: 'pointer', textAlign: 'center', textDecoration: 'none' }}>
                Ver expediente
              </a>
              <a href={`/calculadora?cliente=${modal.cliente.id}`} style={{ flex: 1, padding: '9px', background: AZUL, color: 'white', border: 'none', borderRadius: '10px', fontSize: 'clamp(11px, 0.80vw, 13px)', fontWeight: '600', cursor: 'pointer', textAlign: 'center', textDecoration: 'none' }}>
                Abrir calculadora
              </a>
            </div>
          </div>
      ...
      {showNuevo && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(16px, 2.2vw, 28px)', width: 'min(100%, 460px)', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', color: '#374151', marginBottom: '5px' }}>Cobrado ($)</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 'clamp(12.5px, 0.94vw, 15px)', fontWeight: '700', color: '#374151', marginBottom: '5px' }}>Pago</label>
