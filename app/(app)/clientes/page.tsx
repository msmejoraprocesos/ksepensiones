          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', flex: 1, minWidth: '820px' }}>
            ...
            {[
              ...
              <div key={i} style={{ background: '#FFFFFF', border: '1px solid #E1E7F0', borderRadius: '12px', boxShadow: '0 1px 3px rgba(19,33,53,0.06)', padding: '12px 16px', minWidth: '132px', textAlign: 'center' as const, flexShrink: 0 }}>
                ...
                <div style={{ fontSize: 'clamp(15px, 1.15vw, 18px)', fontWeight: 700, color: k.color, letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' as const, whiteSpace: 'nowrap' as const }}>{k.value}</div>
              </div>
            ))}
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '30px 18px' }}>
              <div style={{ width: 56, height: 56, borderRadius: 999, background: '#F5F7FA', margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(17px, 1.35vw, 22px)', color: '#66738A' }}>◎</div>
                      ...
                      <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f1f5f9' : 'none', cursor: 'pointer' }} onClick={() => openExpediente(c)}>
                        <td style={{ padding: '10px 12px', border: '1px solid #E1E7F0' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <div>
                              <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', color: '#1e293b' }}>{c.nombre}</div>
                              <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>{c.email ?? c.telefono ?? '—'}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <tr key={c.id} style={{ borderBottom: i < clientesArchivados.length - 1 ? '1px solid #f1f5f9' : 'none', opacity: 0.6 }}>
                          <td style={{ padding: '10px 12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              <div>
                                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', color: '#64748b' }}>{c.nombre}</div>
                                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>{c.email ?? c.telefono ?? '—'}</div>
                              </div>
                            </div>
                          </td>
                        ...
                        <div key={cliente.id}
                          ...
                          {(cliente.total_pagado ?? 0) > 0 && (
                            <div style={{ marginTop: '4px', padding: '4px 6px', background: '#f0fdf4', borderRadius: '10px', border: '1px solid #bbf7d0' }}>
                              <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: VERDE, fontWeight: '700' }}>
                                💰 {fmtMXN(cliente.total_pagado ?? 0)} pagado
                              </div>
                              {cliente.monto_acordado && (
                                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>
                                  de {fmtMXN(cliente.monto_acordado)} acordado
                                </div>
                              )}
                            </div>
                          )}
                          <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8', marginTop: '4px' }}>📅 {fmtDias(cliente.ultimo_contacto ?? cliente.created_at)}</div>
                        </div>
                      )
                    })}
                    {cards.length === 0 && (
                      <div style={{ border: `2px dashed ${isDragOver && canDrop ? col.color : '#e2e8f0'}`, borderRadius: '10px', padding: '16px 8px', textAlign: 'center', color: isDragOver && canDrop ? col.color : '#cbd5e1', fontSize: 'clamp(11px, 0.78vw, 12.5px)' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', backdropFilter: 'blur(2px)' }}
          ...
          <div style={{ width: '100%', maxWidth: 'min(720px, 88vw)', maxHeight: '88vh', overflowY: 'auto', height: '90vh', background: 'white', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.25)', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '10px 22px', borderBottom: '1px solid #f1f5f9' }}>
              <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#94a3b8', marginBottom: '5px' }}>Etapa del pipeline</label>
              {editando ? (
                <select
              ...
              {!editando && <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8', margin: '4px 0 0' }}>Activa el modo edición para cambiar la etapa</p>}
            </div>
                  ...
                  {!editando ? (
                    <>
                      {[
                        ...
                        <div key={i}>
                          <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#94a3b8', marginBottom: '3px' }}>{f.label}</div>
                          <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#1e293b' }}>{f.value}</div>
                        </div>
                      ))}
                    </>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: AZUL, margin: 0 }}>Editar datos del cliente</p>
                      {[
                        ...
                        <div key={i}>
                          <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>{f.label}</label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>Notas</label>
                      ...
                      <div style={{ background: '#F4F6F9', borderRadius: '10px', padding: '12px' }}>
                        ...
                        {editando ? (
                          ...
                          <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#1E293B', padding: '7px 0' }}>
                      ...
                      {selected.tipo_servicio && (
                        <div style={{ background: '#F4F6F9', borderRadius: '10px', padding: '12px' }}>
                          <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#94a3b8', marginBottom: '8px' }}>Esquema de pago</div>
                          {selected.tipo_servicio === 'asesoria' ? (
                            editando ? (
                              <div>
                                <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>Monto acordado ($)</label>
                              ...
                              <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#1E293B' }}>{fmtMXN(selected.monto_acordado)}</div>
                            )
                          ) : editando ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              <div>
                                <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>Esquema</label>
                                ...
                                {pagos.length > 0 && (
                                  <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#92400e', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '10px', padding: '6px 8px', margin: '6px 0 0', lineHeight: 1.5 }}>
                              ...
                              {selected.esquema_pago === 'meses_pension' && (
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '8px' }}>
                                  <div>
                                    <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>Pensión mensual ($)</label>
                                  ...
                                  <div>
                                    <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>Meses a cobrar</label>
                              ...
                              {selected.esquema_pago === 'porcentaje_recuperado' && (
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '8px' }}>
                                  <div>
                                    <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>% a cobrar</label>
                                  ...
                                  <div>
                                    <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>Monto recuperado ($)</label>
                            ...
                            <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#1E293B' }}>
                  ...
                  {!editando ? (
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => abrirEditar(selected)}
                        style={{ flex: 1, padding: '9px', background: '#F4F6F9', color: AZUL, border: '1px solid #e2e8f0', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer' }}>
                        ✏️ Editar datos
                      </button>
                      <button onClick={() => setShowConfirmDelete(true)}
                        style={{ padding: '9px 14px', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer' }}>
                        📦 Archivar
                      </button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => setEditando(false)} style={{ flex: 1, padding: '9px', background: '#F4F6F9', color: '#64748b', border: '1px solid #e2e8f0', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer' }}>Cancelar</button>
                      <button onClick={guardarEdicion} style={{ flex: 2, padding: '9px', background: AZUL, color: 'white', border: 'none', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer' }}>Guardar cambios</button>
                    </div>
                  )}
                </div>
              )}
              ...
              {modalTab === 'financiamiento' && (() => {
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    <div>
                      <label style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#475569', display: 'block', marginBottom: '6px' }}>
                    ...
                    {diagSel && (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '10px' }}>
                        <div style={{ background: '#F5F7FA', borderRadius: '10px', padding: '12px', border: '1px solid #e2e8f0' }}>
                          ...
                          <p style={{ fontSize: 'clamp(15px, 1.15vw, 18px)', fontWeight: '700', color: '#245287', margin: 0 }}>{fmtM(capitalBase)}</p>
                        ...
                        <div style={{ background: '#F0FDF4', borderRadius: '10px', padding: '12px', border: '1px solid #bbf7d0' }}>
                          ...
                          <p style={{ fontSize: 'clamp(15px, 1.15vw, 18px)', fontWeight: '700', color: '#2E8B57', margin: 0 }}>{fmtM(pensionBase)}/mes</p>
                    ...
                    {!diagSel ? null : financieras.length === 0 ? (
                      ...
                      <>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '10px' }}>
                          <div>
                            <label style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#475569', display: 'block', marginBottom: '4px' }}>Financiera aliada</label>
                            <select value={finSelId} onChange={e => setFinSelId(e.target.value)}
                              style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e8f0', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontFamily: 'inherit' }}>
                              {financieras.map(f => <option key={f.id} value={f.id}>{f.nombre} — {f.tasa_anual}% anual</option>)}
                            </select>
                          </div>
                          <div>
                            <label style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#475569', display: 'block', marginBottom: '4px' }}>Plazo</label>
                                ...
                                <thead>
                                  <tr style={{ background: '#F4F6F9' }}>
                                    {['#', 'Cuota', 'Capital', 'Interés', 'Saldo'].map((h, i) => (
                                      <th key={i} style={{ padding: '7px 10px', textAlign: i === 0 ? 'center' : 'right', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#64748b', borderBottom: '1px solid #e2e8f0' }}>{h}</th>
                                    ))}
                                  </tr>
                                </thead>
                                <tbody>
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            <div>
                              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8', marginBottom: '4px' }}>
                              ...
                              {pago.pagado && (
                                pago.comprobante_url ? (
                                  <a href={pago.comprobante_url} target="_blank" rel="noopener noreferrer"
                                    style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: AZUL, textDecoration: 'none', background: '#EEF2F8', padding: '3px 8px', borderRadius: '10px', fontWeight: '600' }}>
                                    📎 Ver
                                  </a>
                                ) : (
                                  <label style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8', cursor: 'pointer', background: '#F4F6F9', padding: '3px 8px', borderRadius: '10px', border: '1px dashed #e2e8f0' }}>
                    ...
                    <>
                      ...
                      {(() => {
                        ...
                        return (
                          <div style={{ display: 'flex', gap: '8px' }}>
                            {liquidado ? (
                              <div style={{ flex: 1, padding: '11px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '10px', textAlign: 'center', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: VERDE }}>
                                🟢 Cuenta liquidada — pago completo
                              </div>
                            ) : (
                              <button onClick={() => { setTipoMovimiento('pago'); setFormPago(p => ({ ...p, concepto })); setShowPago(true) }}
                                style={{ flex: 1, padding: '11px', background: VERDE, color: 'white', border: 'none', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer' }}>
                            ...
                            {(selected.total_pagado ?? 0) > 0 && (
                              <button onClick={() => { setTipoMovimiento('devolucion'); setFormPago(p => ({ ...p, concepto: '', monto: '', notas: '' })); setShowPago(true) }}
                                style={{ padding: '11px 14px', background: '#fef2f2', color: '#ef4444', border: '1px solid #fecaca', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                ↩️ Devolución
                              </button>
                            )}
                          </div>
                  ...
                  <div onClick={e => e.stopPropagation()}
                    ...
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                      <button onClick={() => setConfirmDelPago(null)}
                        style={{ padding: '8px 16px', border: '1.5px solid #e2e8f0', borderRadius: '10px', background: 'white', color: '#64748b', fontWeight: '600', fontSize: 'clamp(11px, 0.78vw, 12.5px)', cursor: 'pointer' }}>
                        Cancelar
                      </button>
                      <button onClick={() => { eliminarPago(confirmDelPago.id, confirmDelPago.monto); setConfirmDelPago(null) }}
                        style={{ padding: '8px 16px', border: 'none', borderRadius: '10px', background: '#ef4444', color: 'white', fontWeight: '700', fontSize: 'clamp(11px, 0.78vw, 12.5px)', cursor: 'pointer' }}>
                        🗑️ Eliminar
                      </button>
                    </div>
                  </div>
              ...
              {modalTab === 'diagnosticos' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <button onClick={() => { setSelected(null); router.push(`/calculadora?cliente=${selected.id}`) }}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px', background: AZUL, color: 'white', borderRadius: '10px', border: 'none', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>
                    🧮 Nuevo diagnóstico para {selected.nombre.split(' ')[0]}
                  </button>
                  {diagnosticos.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '20px', color: '#94a3b8', fontSize: 'clamp(11px, 0.78vw, 12.5px)', background: '#F5F7FA', borderRadius: '10px', border: '1px dashed #e2e8f0' }}>
                    ...
                    <div key={d.id} style={{ background: bgColor, borderRadius: '10px', border: `1.5px solid ${borderColor}`, overflow: 'hidden' }}>
                      ...
                      <div style={{ padding: '12px 14px', borderBottom: `1px solid ${borderColor}`, display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                            {/* Estatus badge */}
                            <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', padding: '2px 8px', borderRadius: '10px', background: esAutorizado ? '#dcfce7' : '#f1f5f9', color: esAutorizado ? '#15803d' : '#64748b' }}>
                              {esAutorizado ? '✅ Autorizado' : '📝 Borrador'}
                            </span>
                            <span style={{ background: d.ley === '73' ? '#EEF2F8' : '#EEF7F1', color: d.ley === '73' ? AZUL : VERDE, fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', padding: '2px 8px', borderRadius: '10px' }}>Ley {d.ley}</span>
                            <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>{fmt(d.created_at)}</span>
                            {esAutorizado && d.fecha_autorizacion && (
                              <span style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#15803d' }}>· Autorizado {fmt(d.fecha_autorizacion)}</span>
                            )}
                      ...
                      <div style={{ padding: '10px 14px' }}>
                        ...
                        {(() => {
                          ...
                          if (!snap) return null
                          return (
                            <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#64748b', marginBottom: '8px', padding: '5px 8px', background: '#F9FAFB', borderRadius: '10px', border: '1px solid #e2e8f0', display: 'flex', gap: '10px', flexWrap: 'wrap' as const }}>
              ...
              {modalTab === 'actividades' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <button onClick={() => setShowNuevaActividad(p => !p)}
                    style={{ padding: '9px', background: NARANJA, color: 'white', border: 'none', borderRadius: '10px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer' }}>
                    + Registrar actividad
                  </button>
                  ...
                  {showNuevaActividad && (
                    <div style={{ background: '#F4F6F9', borderRadius: '10px', padding: '14px', border: '1px solid #e2e8f0' }}>
                      ...
                      <div style={{ marginBottom: '10px' }}>
                        <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '6px' }}>¿Cómo fue el contacto? *</label>
                      ...
                      {formActividad.tipo_contacto && (
                        <div style={{ marginBottom: '10px' }}>
                          <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '6px' }}>¿Cuál fue el resultado?</label>
                          <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '6px' }}>
                      ...
                      {formActividad.resultado && (
                        <div style={{ marginBottom: '10px' }}>
                          <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '6px' }}>¿Cuál es el próximo paso?</label>
                          <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '6px' }}>
                      ...
                      {/* Nota opcional */}
                      <div style={{ marginBottom: '10px' }}>
                        <label style={{ display: 'block', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', marginBottom: '4px' }}>Nota adicional (opcional)</label>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                        ...
                        {a.estatus !== 'completado' && (
                          <button onClick={() => completarActividad(a.id)}
                            style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', padding: '2px 7px', borderRadius: '10px', background: '#f0fdf4', color: VERDE, border: '1px solid #bbf7d0', cursor: 'pointer', fontWeight: '600' }}>
                            ✓ Completar
                          </button>
                        )}
                      </div>
          ...
          <div style={{ background: 'white', width: '100%', maxWidth: 'min(440px, 88vw)', maxHeight: '88vh', overflowY: 'auto', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
            <div style={{ background: AZUL, padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => { setShowCanalizarModal(null); setCanalizarDestino(''); setCanalizarMotivo('') }}
                style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', fontSize: 'clamp(15px, 1.15vw, 18px)' }}>✕</button>
            </div>
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column' as const, gap: '14px' }}>
              ...
              <div>
                ...
                <textarea value={canalizarMotivo} onChange={e => setCanalizarMotivo(e.target.value)}
                  ...
                  rows={3}
                  style={{ width: '100%', padding: '8px 10px', border: '1px solid #D1D5DB', fontSize: 'clamp(11px, 0.78vw, 12.5px)', borderRadius: '10px', fontFamily: 'inherit', resize: 'none' as const, boxSizing: 'border-box' as const }} />
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => { setShowCanalizarModal(null); setCanalizarDestino(''); setCanalizarMotivo('') }}
                  style={{ flex: 1, padding: '10px', background: '#F5F7FA', color: '#374151', border: '1px solid #E5E7EB', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px' }}>
                  Cancelar
                </button>
                <button disabled={!canalizarDestino || !canalizarMotivo.trim() || canalizarLoading}
                  ...
                  style={{ flex: 2, padding: '10px', background: !canalizarDestino || !canalizarMotivo.trim() ? '#E1E7F0' : AZUL, color: !canalizarDestino || !canalizarMotivo.trim() ? '#9CA3AF' : 'white', border: 'none', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: !canalizarDestino || !canalizarMotivo.trim() ? 'not-allowed' : 'pointer', fontFamily: 'inherit', borderRadius: '10px' }}>
              ...
              {!encuestaLink ? (
                <>
                  <div style={{ background: '#F5F7FA', borderRadius: '10px', padding: '14px' }}>
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#1E293B', margin: '0 0 8px', fontWeight: '600' }}>La encuesta incluye:</p>
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#66738A', margin: '4px 0' }}>⭐ Calificación del servicio (1-5 estrellas)</p>
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#66738A', margin: '4px 0' }}>🔄 ¿Recomendarías el servicio?</p>
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#66738A', margin: '4px 0' }}>💬 Comentarios (opcional)</p>
                  </div>
                  <button onClick={async () => {
                ...
                <>
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(encuestaLink) }}
                    style={{ padding: '10px', background: 'white', color: '#374151', border: '1px solid #E5E7EB', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit', borderRadius: '10px' }}>
                    📋 Copiar link
                  </button>
                </>
              )}
          ...
          <div style={{ background: 'white', borderRadius: '12px', padding: 'clamp(12px, 1.5vw, 18px)', width: 'min(100%, 480px)', boxShadow: '0 8px 40px rgba(0,0,0,0.2)' }}>
            ...
            {materiales.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '20px', background: '#F4F6F9', borderRadius: '10px', color: '#94a3b8', fontSize: 'clamp(11px, 0.78vw, 12.5px)', marginBottom: '16px' }}>
                <div style={{ fontSize: 'clamp(18px, 1.55vw, 25px)', marginBottom: '8px' }}>📚</div>
                ...
                {materiales.filter(m => m.activo).map(m => {
                  ...
                  return (
                    <label key={m.id}
                      ...
                      <span style={{ fontSize: 'clamp(15px, 1.15vw, 18px)' }}>{emoji}</span>
                      <div style={{ flex: 1 }}>
              ...
              {[
                ...
                <div key={i} style={{ border: '1px solid #e2e8f0', borderRadius: '10px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ padding: '8px 6px', flex: 1 }}>
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#374151', margin: '0 0 4px', lineHeight: 1.3 }}>{col.desc}</p>
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#64748b', margin: 0, lineHeight: 1.3 }}>{col.sub}</p>
                  </div>
                </div>
              ))}
            </div>
...
{/* ── MODAL CONFIRMAR ETAPA ── */}
      ...
      {showConfirmArchivar && selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '14px', padding: 'clamp(12px, 1.5vw, 18px)', width: '100%', maxWidth: 'min(400px, 88vw)', maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowConfirmArchivar(false)}
                style={{ flex: 1, padding: '10px', border: '1px solid #e2e8f0', borderRadius: '10px', background: '#F4F6F9', color: '#64748b', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}>
                Cancelar
              </button>
              <button onClick={() => { setShowConfirmArchivar(false); archivarCliente() }} disabled={deletingCliente}
                style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '10px', background: '#dc2626', color: 'white', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>
                {deletingCliente ? 'Archivando...' : '📦 Sí, archivar'}
              </button>
            </div>
          </div>
            ...
            return (
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                <div style={{ flex: 1, background: '#f0fdf4', borderRadius: '10px', padding: '8px 12px' }}>
                  <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>Total pagado</div>
                  <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: VERDE }}>{fmtMXN(selected.total_pagado ?? 0)}</div>
                </div>
                <div style={{ flex: 1, background: '#fef2f2', borderRadius: '10px', padding: '8px 12px', border: '1px solid #fecaca' }}>
                  <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>Devolución máxima</div>
                  <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '800', color: '#ef4444' }}>{fmtMXN(selected.total_pagado ?? 0)}</div>
                </div>
              </div>
            )
          }
          return (
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
              <div style={{ flex: 1, background: '#F4F6F9', borderRadius: '10px', padding: '8px 12px' }}>
                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>Acordado</div>
                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: AZUL }}>{fmtMXN(selected.monto_acordado)}</div>
              </div>
              <div style={{ flex: 1, background: '#f0fdf4', borderRadius: '10px', padding: '8px 12px' }}>
                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>Pagado</div>
                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: VERDE }}>{fmtMXN(selected.total_pagado ?? 0)}</div>
              </div>
              <div style={{ flex: 1, background: saldo > 0 ? '#fff5f5' : '#f0fdf4', borderRadius: '10px', padding: '8px 12px', border: `1px solid ${saldo > 0 ? '#fecaca' : '#bbf7d0'}` }}>
                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#94a3b8' }}>Saldo máximo</div>
                <div style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '800', color: saldo > 0 ? '#ef4444' : VERDE }}>{fmtMXN(saldo)}</div>
              </div>
            </div>
          )
        })()}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                ...
                {tipoMovimiento === 'devolucion' ? (
                  formPago.monto && parseFloat(formPago.monto) > (selected.total_pagado ?? 0) && (
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#ef4444', margin: '3px 0 0' }}>
              ...
              <div>
                ...
                {compFile && (
                  <button onClick={() => setCompFile(null)} style={{ marginTop: '4px', fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                    ✕ Quitar archivo
                  </button>
                )}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div>
                ...
                {nssAlerta && (
                  <div style={{ marginTop: '6px', padding: '10px 12px', background: nssAlerta.tipo === 'activo' ? '#FEF2F2' : '#EFF6FF', border: `1px solid ${nssAlerta.tipo === 'activo' ? '#FCA5A5' : '#93C5FD'}`, borderRadius: '10px' }}>
                    <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: nssAlerta.tipo === 'activo' ? '#991B1B' : '#1D4ED8', fontWeight: '600', margin: '0 0 6px' }}>{nssAlerta.mensaje}</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '10px' }}>
                ...
                <div>
                  ...
                  {formErrors.tipo_servicio && <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#ef4444', margin: '3px 0 0' }}>⚠️ {formErrors.tipo_servicio}</p>}
                </div>
              </div>
              {form.tipo_servicio === 'asesoria' ? (
                ...
                <div>
                  ...
                  {formErrors.esquema_pago && <p style={{ fontSize: 'clamp(11px, 0.78vw, 12.5px)', color: '#ef4444', margin: '3px 0 0' }}>⚠️ {formErrors.esquema_pago}</p>}
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(12px, 0.90vw, 14px)' }}>
                        <thead>
                          <tr style={{ background: '#F4F6F9' }}>
                            ...
                            <th style={{ padding: '6px 10px', textAlign: 'center', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#64748b' }}>Cobrar</th>
                            <th style={{ padding: '6px 10px', textAlign: 'right', fontSize: 'clamp(11px, 0.78vw, 12.5px)', fontWeight: '700', color: '#64748b' }}>Monto ($)</th>
                          </tr>
                        </thead>
                        <tbody>
...
export default function ClientesPage() {
  return (
    <Suspense fallback={<div style={{ padding: '24px 18px', textAlign: 'center', color: '#94a3b8' }}>Cargando...</div>}>
      <ClientesInner />
    </Suspense>
  )
}
